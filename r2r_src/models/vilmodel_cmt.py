...
class LXRTXLayer(nn.Module):
    ...
    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        ...
        return lang_att_output, visn_att_output, attention_scores
    ...
    def forward(self, lang_feats, lang_attention_mask,
                      visn_feats, visn_attention_mask, return_lang_att_scores=False):
        lang_att_output = lang_feats
        visn_att_output = visn_feats

        # if return_lang_att_scores:
        #     import pdb; pdb.set_trace();

        lang_att_output, visn_att_output, cross_attention_scores = self.cross_att(lang_att_output, lang_attention_mask,
        ...
        if return_lang_att_scores:
            return lang_output, visn_output, language_attention_scores
        else:
            return lang_output, visn_output
...
class DSTVisionEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.img_linear = nn.Linear(config.image_feat_size, config.hidden_size)
        self.img_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.ang_linear = nn.Linear(config.angle_feat_size*32, config.hidden_size) # config.angle_feat_size=4
        self.ang_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        # 0: non-navigable, 1: navigable, 2: stop
        self.nav_type_embedding = nn.Embedding(3, config.hidden_size)
        ...
        self.act_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, img_feat, ang_feat, type_embeddings, nav_types=None):
        transformed_im = self.img_layer_norm(self.img_linear(img_feat))
        transformed_ang = self.ang_layer_norm(self.ang_linear(ang_feat.repeat(1, 1, 32)))
        obj_embeddings = transformed_im + type_embeddings
        act_embeddings = transformed_ang + type_embeddings
        if nav_types is not None:
            nav_embeddings = self.nav_type_embedding(nav_types)
            obj_embeddings = obj_embeddings + nav_embeddings
            act_embeddings = act_embeddings + nav_embeddings

        obj_embeddings = self.obj_layer_norm(obj_embeddings)
        obj_embeddings = self.dropout(obj_embeddings)
        act_embeddings = self.act_layer_norm(act_embeddings)
        act_embeddings = self.dropout(act_embeddings)
        return obj_embeddings, act_embeddings
...
class NavCMT(BertPreTrainedModel):
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
        ...
        # cross-modal encoding per step
            ...
            # print('self.config.no_lang_ca:', self.config.no_lang_ca)
            # print('self.config.act_pred_token:', self.config.act_pred_token)
...
class NavCMT_DST(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.img_embeddings = ImageEmbeddings(config)
        ...
        self.obj_next_action = NextActionPrediction(config.hidden_size, config.pred_head_dropout_prob)
        self.act_next_action = NextActionPrediction(config.hidden_size, config.pred_head_dropout_prob)

        # DST needs
        self.obj_addlayer = nn.ModuleList(
            [LXRTXLayer(config) for _ in range(config.num_dst_layer)])
        self.act_addlayer = nn.ModuleList(
            [LXRTXLayer(config) for _ in range(config.num_dst_layer)])
        self.config = config
        self.dst_vision_encoder = DSTVisionEncoder(config)
        self.logit_fc = nn.Linear(config.hidden_size, 2) 
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
        ...
        # cross-modal encoding per step
            ...
            if self.config.no_lang_ca:
                obj_txt_embeds = txt_embeds[-1]
                act_txt_embeds = txt_embeds[-1]
            else:
                obj_txt_embeds = txt_embeds
                act_txt_embeds = txt_embeds
            ...
            # DST begin
            new_ob_token_type_ids = torch.ones(ob_img_feats.size(0), ob_img_feats.size(1), dtype=torch.long, device=self.device)
            obj_ob_embeds, act_ob_embeds = self.dst_vision_encoder(ob_img_feats, ob_ang_feats, 
                self.embeddings.token_type_embeddings(new_ob_token_type_ids), 
                nav_types=ob_nav_types)
            obj_hist_ob_embeds = torch.cat([hist_embeds, obj_ob_embeds], 1)
            act_hist_ob_embeds = torch.cat([hist_embeds, act_ob_embeds], 1)
            for l, layer_module in enumerate(self.obj_addlayer):
                obj_txt_embeds, obj_hist_ob_embeds, obj_language_attention_scores = layer_module(
                    obj_txt_embeds, extended_txt_masks, 
                    obj_hist_ob_embeds, extended_hist_ob_masks,
                    return_lang_att_scores=True
                )
            for l, layer_module in enumerate(self.act_addlayer):
                act_txt_embeds, act_hist_ob_embeds, act_language_attention_scores = layer_module(
                    act_txt_embeds, extended_txt_masks, 
                    act_hist_ob_embeds, extended_hist_ob_masks,
                    return_lang_att_scores=True
                )
            obj_ob_embeds = obj_hist_ob_embeds[:, hist_max_len:]
            act_ob_embeds = act_hist_ob_embeds[:, hist_max_len:]


            if self.config.com_weight_from == 'txt_cls':
                inst_drop = self.drop(txt_embeds[:, 0])
            elif self.config.com_weight_from == 'txt_hist_cls':
                inst_drop = self.drop(txt_embeds[:, 0] * hist_embeds[:, 0])
            else:
                assert False, 'com_weight_from is illegal'

            weight = self.logit_fc(inst_drop)

            assert self.config.act_pred_token == 'ob_txt'

            obj_act_logits = self.obj_next_action(obj_ob_embeds * txt_embeds[:, :1]).squeeze(-1)
            act_act_logits = self.act_next_action(act_ob_embeds * txt_embeds[:, :1]).squeeze(-1)

            act_logits = obj_act_logits * weight[:, 0].unsqueeze(1) + act_act_logits * weight[:, 1].unsqueeze(1)

            act_logits.masked_fill_(ob_nav_types==0, -float('inf'))

            obj_language_attention_scores = obj_language_attention_scores.mean(dim=1)
            act_language_attention_scores = act_language_attention_scores.mean(dim=1)

            return act_logits, txt_embeds, hist_embeds, ob_embeds, obj_language_attention_scores, act_language_attention_scores
