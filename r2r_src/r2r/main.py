...
def build_dataset(args, rank=0, is_test=False):
    ...
    return train_env, val_envs, aug_env, tok


def train(args, train_env, val_envs, aug_env=None, rank=-1, tok=None):
...
def valid(args, train_env, val_envs, rank=-1, tok=None):
    default_gpu = is_default_gpu(args)
    ...
    if args.dataset == 'r2r_back':
        ...
        agent_class = Seq2SeqCMTAgent
    agent = agent_class(args, train_env, rank=rank, tok=tok)
    ...
    for env_name, env in val_envs.items():
        ...
        if 'train' in env_name:
            continue
        if 'val_seen' == env_name:
            continue

        print('Now valid ', env_name)
...
def train_laspeaker(args, train_env, tok=None, n_iters=160000, log_every=500, val_envs={}):
    # writer = SummaryWriter(logdir=log_dir)
    if tok is None:
        tok = get_tokenizer(args)
    speaker = LA_Speaker(train_env, tok)
    ...
    for idx in range(0, n_iters, log_every):
        ...
        for env_name, env in val_envs.items():
            ...
            if 'val_seen' in env_name: # Ignore the val seen set for the efficiency
                continue
            ...
            if loss < best_loss[env_name]:
                ...
                speaker.save(idx, os.path.join(args.ckpt_dir, 'laspeaker', 'best_%s_loss' % env_name))
...
def main():
    ...
    train_env, val_envs, aug_env, tok = build_dataset(args, rank=rank)

    if args.train == 'laspeaker':
        train_laspeaker(args, train_env, tok=tok, val_envs=val_envs)
    elif not args.test:
        train(args, train_env, val_envs, aug_env=aug_env, rank=rank, tok=tok)
    else:
        valid(args, train_env, val_envs, rank=rank, tok=tok)
