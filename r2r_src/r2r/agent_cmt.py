...
from utils.misc import length2mask, remove_nan
...
from r2r.la_speaker import LA_Speaker
...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0, tok=None):
        ...
        self.laspeaker = None
        if args.laspeaker is not None:
            self.laspeaker = LA_Speaker(self.env, tok)
            print("Load the laspeaker from %s." % args.laspeaker)
            self.laspeaker.load(args.laspeaker)
    ...
    def _candidate_variable_of_GT(self, obs, actions):
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        Pred_obj_weight = []
        Pred_act_weight = []
        lengths = np.zeros(len(obs), np.int64)
        img_feats = []
        can_feats = []
        ...
        for t in range(self.args.max_action_len):
            ...
            Pred_obj_weight.append(t_outputs[-2])
            Pred_act_weight.append(t_outputs[-1])
            ...
            # landmark and action loss
            if self.feedback == 'teacher' and self.env.name == 'train':
                if abs(args.LWeight) > 1e-3:
                    
                    obj_weight = t_outputs[-2]
                    act_weight = t_outputs[-1]
                    
                    landmark = torch.zeros_like(obj_weight).cuda()
                    action = torch.zeros_like(act_weight).cuda()
                    land_mask = np.zeros(batch_size, np.float32)
                    act_mask = np.zeros(batch_size, np.float32)

                    for i, ob in enumerate(obs):
                        land_id = torch.LongTensor(ob['landmark']).cuda()
                        act_id = torch.LongTensor(ob['action']).cuda()
                        if land_id[0] >= 0:
                            land_mask[i] = 1
                            landmark[i][land_id+1] = 1   # +1: due to [CLS] token
                        if act_id[0] >= 0:
                            act_mask[i] = 1
                            action[i][act_id+1] = 1
                    ...
                    t_landmark_loss = F.binary_cross_entropy_with_logits(obj_weight, landmark, reduce=False) * \
                                    torch.from_numpy(land_mask).cuda().unsqueeze(1)
                    landmark_loss += torch.mean(remove_nan(t_landmark_loss))

                    t_action_loss = F.binary_cross_entropy_with_logits(act_weight, action, reduce=False) * \
                                    torch.from_numpy(act_mask).cuda().unsqueeze(1)
                    action_loss += torch.mean(remove_nan(t_action_loss))
            ...
            if self.env.name == 'aug' or self.feedback == 'sample':
                feat_t = self._feature_variable(obs)
                cand_t = self._candidate_variable_of_GT(obs, cpu_a_t)
                # # Apply the env drop mask to the feat
                # if self.env.name == 'aug' and args.drop_env:
                #     cand_t[..., :-args.angle_feat_size] *= noise
                #     feat_t[..., :-args.angle_feat_size] *= noise
                img_feats.append(feat_t)
                can_feats.append(cand_t)
                lengths += (1 - ended)
            ...
            if train_rl or ((not np.logical_or(ended, (cpu_a_t == -1)).all()) and (t != self.args.max_action_len-1)):
                ...
                for i, next_id in enumerate(cpu_a_t):
                    # print('?' * 10, i, next_id, len(obs[i]['candidate']))
                    # print(cpu_a_t)
        ...
        # Landmark/Action Loss : RL + IL-BT
        if (self.env.name == 'aug' or self.feedback == 'sample'):
            if self.laspeaker is not None:
                ...
                # if args.mask_la:
                if False:
                    GT_land_weight, GT_act_weight, land_mask, act_mask = self.laspeaker.infer_batch(insts=insts, features=features, for_il=(self.feedback == 'teacher'))
                else:
                    GT_land_weight, GT_act_weight = self.laspeaker.infer_batch(insts=insts, features=features)
                    land_mask = ~ctx_mask
                    act_mask = ~ctx_mask
                
                Pred_act_weight = torch.stack(Pred_act_weight, 1).contiguous()
                Pred_obj_weight = torch.stack(Pred_obj_weight, 1).contiguous()
                # import pdb; pdb.set_trace();
                right = Pred_obj_weight.size(2)  # due to the output of encoder is not always b x seq x 60
                GT_land_weight = GT_land_weight[..., :right]
                GT_act_weight = GT_act_weight[..., :right]

                t_landmark_loss = F.binary_cross_entropy_with_logits(Pred_obj_weight, GT_land_weight, reduce=False) * \
                                            (land_mask).unsqueeze(2).float()
                landmark_loss += torch.mean(remove_nan(t_landmark_loss))

                t_action_loss = F.binary_cross_entropy_with_logits(Pred_act_weight, GT_act_weight, reduce=False) * \
                                            (act_mask).unsqueeze(2).float()
                action_loss += torch.mean(remove_nan(t_action_loss))

        if args.norm_la_weight and (type(landmark_loss) is not float):
            landmark_loss = landmark_loss / (landmark_loss.item() + 1e-12)
            action_loss = action_loss / (action_loss.item() + 1e-12)

        self.loss += landmark_loss * args.LWeight
        self.loss += action_loss * args.AWeight
