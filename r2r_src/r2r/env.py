...
class R2RBatch(object):
    ...
    def __init__(
        ...
        if self.name == 'train':
            self.landmark_action = json.load(open(args.train_la_path,'r'))
            print("load training set landmark and action of %d paths" % len(self.landmark_action))
        elif self.name == "val_unseen":
            self.landmark_action = json.load(open(args.val_unseen_la_path,'r'))
            print("load val unseen set landmark and action of %d paths" % len(self.landmark_action))
        elif self.name == "val_seen":
            self.landmark_action = json.load(open(args.val_seen_la_path,'r'))
            print("load val seen set landmark and action of %d paths" % len(self.landmark_action))
        else:
            self.landmark_action = {}
    ...
    def get_point_dis(self, scanId, viewpoint1, viewpoint2):
        return len(self.shortest_paths[scanId][viewpoint1][viewpoint2])
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            # landmark and action label
            inst_length = (np.array(item['instr_encoding']) != padding_idx).sum()
            if True:
                obs[-1]['landmark'] = []
                obs[-1]['action'] = []
                # inst_length = (item['instr_encoding'] != padding_idx).sum().item()
                if str(item['path_id']) in self.landmark_action.keys():
                    LA = self.landmark_action[str(item['path_id'])]
                    # import pdb; pdb.set_trace();
                    if str(obs[-1]['viewpoint']) in LA:
                        if str(item['instr_id'][-1]) in LA[str(obs[-1]['viewpoint'])]:
                            t = LA[str(obs[-1]['viewpoint'])][str(item['instr_id'][-1])]
                            if 'landmark' in t.keys():
                                for i in t['landmark']:
                                    if i >= 0 and i < inst_length-1: # filter noisy data !!!!!!!
                                        obs[-1]['landmark'].append(i)
                            if 'action' in t.keys():
                                for i in t['action']:
                                    if i >= 0 and i < inst_length-1: # filter noisy data !!!!!!!
                                        obs[-1]['action'].append(i)
                if len(obs[-1]['landmark']) == 0:
                    obs[-1]['landmark'] = [-1]
                if len(obs[-1]['action']) == 0:
                    obs[-1]['action'] = [-1]
        return obs
